...
const WeekdayAttendanceChart = () => {
  ...
  // Process data to count on-time vs late arrivals by weekday
  const processWeekdayData = () => {
    const weekdayNames = ['Domingo', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado'];
    
    // Initialize data structure for each weekday
    const weekdayData = weekdayNames.map(name => ({
      name,
      onTime: 0,
      late: 0
    }));
    ...
    entryLogs.forEach(log => {
      ...
      if (hours < 8 || (hours === 8 && minutes === 0)) {
        weekdayData[weekday].onTime++;
      } else {
        weekdayData[weekday].late++;
      }
    });
    
    // Only include weekdays with data (typically Monday-Friday)
    return weekdayData.filter(day => day.onTime > 0 || day.late > 0);
  };

  const weekdayData = processWeekdayData();
  
  // Prepare data for Chart.js
  const labels = weekdayData.map(day => day.name);
  const onTimeData = weekdayData.map(day => day.onTime);
  const lateData = weekdayData.map(day => day.late);
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Asistencia por Día de la Semana',
        color: 'white',
        font: {
          size: 16
        }
  ...
  // Handle click on weekday to navigate to tables view
  const handleWeekdayClick = (weekday, type) => {
    navigate(`/dashboard/reportes/tablas?category=weekday&type=${weekday}&value=${type}`);
  };

  // Handle click on total statistics
  const handleTotalClick = (type) => {
    navigate(`/dashboard/reportes/tablas?category=weekday&value=${type}`);
  };
...
export default WeekdayAttendanceChart;
